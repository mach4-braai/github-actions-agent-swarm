...
| Misplaced concurrency or `fail-fast: true` destroys slices | Both cancel rather than delay, and both look like ordinary CI hygiene. Preflight requires the workflow-level group with `queue: max` and rejects any job-level group or `cancel-in-progress` (D1); `fail-fast: false` is mandatory and asserted by AC12 |
